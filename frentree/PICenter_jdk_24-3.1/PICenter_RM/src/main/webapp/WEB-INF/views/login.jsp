...
$(document).ready(function () {
	...
	var pic_ip = <%= "\"" + request.getHeader("host") +"\""%>
	
	if(pic_ip.indexOf("") != -1){
	}else{
		var sso_sabun = <%= request.getHeader("SSO_SABUN")%>;
		if(sso_sabun == null){ 
			portalLogin();
		}else{
			portalSSOLogin(sso_sabun);
		} 
	}
	// 쿠키에 저장된 아이디 정보
...
function portalSSOLogin(sso_sabun){
	...
	$.ajax({
			type: "POST",
			url: "samsung/SSOGradeCheck",
			async : false,
			data : postData,
		    success: function (resultMap) {
		    	
		    	if(resultMap.resultCode == -1 ){
		    		$("#notPotalLoginTXT").html(resultMap.resultMessage);
		    		$("#notPortalLogin").show();
		    	}else if(resultMap.resultCode == -100){
		    		$("#notPotalLoginTXT").html(resultMap.resultMessage);
		    		$("#notPortalLogin").show();
		    	}else{
		    		if(resultMap.user_grade !=9 && resultMap.user_grade != null){
		    			SSOLogin(sso_sabun);
		    		}
		    	}
...
function SSOLogin(sso_sabun){
	...
	$.ajax({
			type: "POST",
			url: "samsung/SSOLogin",
			async : false,
			data : postData,
		    success: function (resultMap) {
		    	if(resultMap.resultCode == -100){
		    		$("#notPotalLoginTXT").html(resultMap.resultMessage);
		    		$("#notPortalLogin").show();
		    	}else if(resultMap.resultCode == -3 || resultMap.resultCode == -2){
		    		alert(resultMap.resultMessage);
		    		return;
		    	}else{
		    		successLogin();
