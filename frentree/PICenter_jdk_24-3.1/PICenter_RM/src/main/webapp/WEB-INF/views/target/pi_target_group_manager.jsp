...
<div id="taskGroupWindow" class="ui-widget-content" style="position:absolute; left: 10px; top: 10px; touch-action: none; width: 150px; z-index: 999; 
	...
	<ul>
		<li class="status">
			<button id="viewServer">조회</button></li>
		<li class="status">
			<button id="addServer">수정</button></li>
		<li class="status">
			<button id="closeManagerAll">닫기</button></li>
	</ul>
	<input type="hidden" id = 'view_server'>
</div>
...
<div id="pcAdminExcelPopup" class="popup_layer" style="display:none">
	<div class="popup_box" style="width: 620px; height: 540px; padding: 10px; background: #f9f9f9; left: 50%; top: 51%;">
		...
		<div class="popup_top" style="background: #f9f9f9;">
			<h1 style="color: #222; padding: 0; box-shadow: none;">진단담당자 일괄 등록</h1>
						...
						<tr>
							...
							<td>
								<form id="btnDownLoadXlsx" action="<%=request.getContextPath()%>/download/downloadExcel" method="post" style="display: inline; width: 49px; padding: 0px;">
									<input id="downloadFile" type="hidden" name="filename" value="PIMC_진단담당자_관리.xlsx">
								...
								<button type="button" id="clickinsaCodeBtn" style="margin: 2px 0 2px 7px; width: 115px;">인사코드 조회</button>
							</td>
						</tr>
					</tbody>
				</table>
					...
					<tbody >
						<tr>
							...
							<td>
								...
								<input type="text" id="importExcelNm" style="width: 370px; font-size: 12px; padding: 0 0 0 5px; margin: 0 0 0 7px;" readonly="">
							</td>
						</tr>
					</tbody>
				</table>
...
$(document).ready(function() {
	...
	$("#viewServer").click(function(e){
		
		var row = $("#topNGrid").getGridParam("selrow");
	    var server = $("#topNGrid").getCell(row, "SERVER");
	    var insa_code = $("#topNGrid").getCell(row, "INSA_CODE");
	   
	    $("#view_server").val(server,insa_code);
...
function fn_drawTopNGrid() {
	...
	colModel.push({ index: 'INSA_CODE', 	name: 'INSA_CODE', 	width: 80, align: "center"});
	colModel.push({ index: 'FULLNAME', 	name: 'FULLNAME', 	width: 130, align: "left"});
	colModel.push({ index: 'TEAM_NAME', 	name: 'TEAM_NAME', 	width: 100, align: "center"});
	colModel.push({ index: 'FULLNAME', 	name: 'FULLNAME', 	width: 100, align: "center", hidden:true });
	colModel.push({ index: 'CNT', 	name: 'CNT', width: 50, align: "center"});
	colModel.push({ index: 'SERVER', 	name: 'SERVER', width: 100, align: "center",hidden:true});
	colModel.push({ index: 'VIEW', 		name: 'VIEW', width:30, align: 'center', formatter:createView, exportcol : false, type:0});
	
	$("#topNGrid").jqGrid({
		url: "<%=request.getContextPath()%>/target/selectGroupManagerList",
		datatype: "json",
		//data: temp,
	   	mtype : "POST",
		colNames:['인사코드','부서','팀명','담당 팀','소속 서버','서버', ''],
		colModel: colModel,
		loadonce:true,
	   	autowidth: false ,
		shrinkToFit:false,
		...
		loadComplete: function(data) {
			...
			$(".gridSubSelBtn").on("click", function(e) {
				...
				$("#taskGroupWindow").css("left", (offset.left - $("#taskGroupWindow").width()) + 24 + "px");
...
$('#searchInsaCode').keyup(function(e) {
	if (e.keyCode == 13) {
		search_Manager();
	}        
});

$('#searchTeamNm').keyup(function(e) {
	if (e.keyCode == 13) {
		search_Manager();
	}        
});
...
function search_Manager(){
	var postData = {
		"insa_code": $('#searchInsaCode').val(), 
		"team_name": $('#searchTeamNm').val()
		/* "team_name": $('#searchTeam').val() */
	};
	$("#topNGrid").setGridParam({url:"<%=request.getContextPath()%>/target/selectGroupManagerList", postData : postData, datatype:"json" }).trigger("reloadGrid");
};
...
$("#clickinsaCodeBtn").click(function(e){
	insaCodeListWindows();
});
...
function insaCodeListWindows(){
	var netid = 99;
	var netType = "insaCode";
...
$("#btnpcAdminPopupSave").click(function(e){
	...
	var message = user_name + "님을 " + team_name + "의 진단담당자로 지정 하시겠습니까?";
        ...
        workBook.SheetNames.forEach(function (sheetName) {
	    	...
	    	if(
	    		...
	    		$.each(rows, function(index, item) {
					...
					if(user_no == 'P123456' && insa_code == '00000001'){
						firstRowChk = true;
					}else if(!firstRowChk) {
	            		...
	            		$.ajax({
	        			    ...
	        			    success: function (resultMap) {
	        			    	...
	        			    	if(resultMap.resultValue == 0) {
	        			    		...
	        			    		alert("해당 담당자들은 이미 해당 팀의 진단담당자로 지정되어 있습니다.");
	        			    	}else {
	        			    		alert(resultMap.resultValue + "명의 담당자는 이미 해당 팀의 진단담당자로 지정되어 있습니다. \n해당 담당자를 제외한 " + resultPerson + "명의 진단담당자 등록을 완료하였습니다.");
		        			    	setNewSetting();
...
function viewServer(flag,insa_code){
	...
	data = document.createElement('input');
	data.setAttribute('type','hidden');
	data.setAttribute('name','insa_code');
	data.setAttribute('value',insa_code);
	newForm.appendChild(data);
