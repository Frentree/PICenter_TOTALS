...
function popSearch(){
	...
	var approvalGroupCnt = '${approvalGroupCnt}'.split('[{').join('').split('}]').join('');
	approvalGroupCnt = approvalGroupCnt.split('}, {');
	
	var teamApprovalStatus = false;
	for(var i = 0; approvalGroupCnt.length > i; i++){ // str 배열만큼 for돌림
		   
		var approvalRow = approvalGroupCnt[i].split(', ');
		var sType = approvalRow[0].split('sType=').join(''); //변수 TABLE_NAME은 TB_NAME1, 다음 반복문에서는 TB_NAME2
		var cnt = approvalRow[1].split('cnt=').join(''); // 변수 CHK Y, 다음 반복문에서는 N
		
		if(sType =="teamStaus") {
			if (cnt > 0 ) { 
				teamApprovalStatus = true;
			}
		}
	}
	...
	if(teamApprovalStatus){
		$("#userGrid").setGridParam({
			url:"<%=request.getContextPath()%>/popup/approvalTeamUserList", 
			postData : postData, 
			datatype:"json" 
			}).trigger("reloadGrid");
