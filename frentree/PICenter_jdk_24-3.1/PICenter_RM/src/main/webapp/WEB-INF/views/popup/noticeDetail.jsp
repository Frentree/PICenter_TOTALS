	...
	if(uploadFile) {
		...
		$.ajax({
			...
			success: function (result) {
				...
				// 파일 업로드 성공 후 noticeUpdate 호출
				updateNotice(result.fileNumber);
				setProgress(0 / 1);
			},
			error: function (e) {
				$('#btnNoticeSave').prop('disabled', false);
		...
		// 파일이 없으면 바로 noticeUpdate 호출 (기존 파일번호 사용)
		updateNotice($("#updateFileNum").val());
	}
});

// noticeUpdate 공통 함수
function updateNotice(fileNumber) {
	var postData = {
