						...
						<tr>
                            ...
                            <td style="padding: 5px 5px 0 5px;">
                                <select id="processingFlag" name="statusList" style="width:186px;">
                                    <option value="">전체</option>
                                    <c:forEach var="trueL" items="${approvalList}" varStatus = "status">
											<option value="${trueL.PROCESSING_FLAG}" >${trueL.APRROVAL_TYPE}${trueL.PROCESSING_FLAG_NAME}</option>
										</c:forEach>
                                </select>
                            </td>
                            <td rowspan="3">
...
<div id="userSelect" class="popup_layer" style="display:none;">
	...
	<img class="CancleImg" id="btnCancleUserSelect" src="${pageContext.request.contextPath}/resources/assets/images/cancel.png">
		<div class="popup_top" style="background: #f9f9f9;">
			<h1 style="color: #222; padding: 0; box-shadow: none; float: left; padding-right: 5px;">기안자 지정</h1>
			<input type="radio" name="apporvalUser" value="approval_6" style="width: 12px; font-size: 12px"> 합의
			<input type="radio" name="apporvalUser" value="approval_7" style="width: 12px; font-size: 12px"> 통보
			<input type="radio" name="apporvalUser" value="approval_8" checked="checked" style="width: 12px; font-size: 12px"> 결재
		</div>
		<div class="popup_content">
...
<%-- 		url: "<%=request.getContextPath()%>/user/approvalList", --%>
		...
		//data: temp,
	   	mtype : "POST",
	   	colNames:['구분', '구분','구분2', '이름', '사번', '직책', '소속','필수', '필수', '', ''],
		colModel: [
			...
			{ index: 'JIKWEE', 			name: 'JIKWEE',			width: 50, 	align: 'center'},
	...
	if(rowId){
		...
		if(prevRowId){
			...
			if(rowData.STATUS != beforeRowData.STATUS){
				alert(rowData.STATUS_NAME + "는 " + beforeRowData.STATUS_NAME+"보다 먼저 올 수 없습니다.");
				return;
			}
...
$("#jqgridDownBtn").click(function(){
	...
	if(rowId){
		...
		if(nextRowId){
			...
			if(rowData.STATUS != beforeRowData.STATUS){
				alert(rowData.STATUS_NAME + "는 " + beforeRowData.STATUS_NAME +"보다 나중에 올 수 없습니다.");
				return;
			}
...
function loadJqGrid(oGrid)
{
    ...
    oGrid.jqGrid({
        ...
        colModel: [
            ...
            {label: '호스트', 	    index: 'NAME',           name: 'NAME',             width: 80,  align: 'center', formatter: hostName},
            {label: '호스트 CNT', 	    index: 'NAME_CNT',       name: 'NAME_CNT',         width: 80, hidden:true},
            ...
            {label: '구분',			index: 'CHARGE_USER_FLAGE', name: 'CHARGE_USER_FLAGE', width: 50,  align: 'center', formatter:formatName},
            {label: '상태1',			index: 'STATUS', 		  name: 'STATUS', 		   width: 50,  align: 'center',  hidden:true},
            {label: '비고',			index: 'NOTE',            name: 'NOTE',            width: 130, align: 'left'},
            {label: 'BASIS',		index: 'NOTEPAD',         name: 'NOTEPAD',           width: 10,  hidden:true},
            ...
            {label: '결재 구분',		index: 'APPROVAL_FLAG',   name: 'APPROVAL_FLAG',   width: 100, align: 'left', hidden:true}
        ],
        loadonce: true,
        viewrecords: true, // show the current page, data rang and total records on the toolbar
                        ...
                        if (searchList.length > 0) {
                            $.each(searchList, function (i, s) {
                            	...
                            	if(oGrid.getCell(nRowid, 'APPROVAL_FLAG') == 1) { // 정/오탐
                            		...
                            		if(firstExcepathId == ExcepathId) exPathex += arr[i].hash_id +"<br>";
                                    ...
                                    if((i+1)==searchList.length){
                                    	 ...
                                    	 $("#excepPath").append(
 	                                    		"<tr name='excepPathAddTr' style='border:none;'>"+
 	                                    			"<th style='padding:0px; background: transparent; text-align: left;'>"+excepath+"경로 : " + exPathex +"</th>" +  
 	                                   			"</tr>"
 	                                   	);
                                    }else{ 
	                                    $("#excepPath").append(
...
function reqApproval()
{
    ...
    for (var i = 0; i < aSelRows.length; i += 1)
    {
        ...
        var approval_flag = $("#processGrid").getCell(aSelRows[i], 'APPROVAL_FLAG');
...
function saveApproval()
{
    ...
    $.ajax({
        type: "POST",
        url: "${getContextPath}/samsung/selectDocuNum",
        async : false,
        data : { "today": sToday },
        datatype: "json",
        success: function (result) {
	...
	$("#userGrid").jqGrid({
        url: null,
 		...
 		colModel: [
 			...
 			{ index: 'JIKWEE', 		name: 'JIKWEE', 	width: 120, align: "left", hidden:true}
 		],
         id: "USER_NO",
         loadonce:true,
         viewrecords: true,
         ...
         ondblClickRow: function(nRowid,iRow,iCol) {
     		...
     		var sJikguk = $("#userGrid").getCell(nRowid, 'JIKWEE'); 
