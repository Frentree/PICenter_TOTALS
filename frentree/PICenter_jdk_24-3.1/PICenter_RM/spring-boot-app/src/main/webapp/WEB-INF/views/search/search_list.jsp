				...
				<div class="content magin_t35" style="height: 740px;">
					<div class="grid_top">  
						<div class="searchBox" style="float: left;">
							<table class="user_info narrowTable" id="navGridSearchDiv"  style="width: 714px;">
								<tbody>
									<tr id="searchTextBox">
...
<div id="viewDetails" class="popup_layer" style="display:none"> 
	<div class="ui-widget-content" id="popup_datatype" style="position:absolute; height: 724px; left: 27%; top: 8%; touch-action: none; max-width: 920px; z-index: 999; background: #f9f9f9;">
	<img class="CancleImg" id="btnCancleViewDetails" src="${pageContext.request.contextPath}/resources/assets/images/cancel.png">
		<div class="popup_container">
			...
			<div class="popup_content">
				<div class="content-box" style="width: 900px !important; background: #fff; border: 1px solid #c8ced3; border-bottom: none; height: 342px;">
					<table class="popup_tbl">
						<colgroup>
							<col width="20%">
							<col width="*">
						...
						<tbody>
							...
							<tr>
								<th>스캔 대수</th>
								<td id="details_cnt"></td>
							</tr>
							<tr>
								<th>전체 진행률</th>
								<td id="details_percentage"></td>
							</tr>
						</tbody>
					</table>
					<table class="popup_tbl">
...
<div id="progressDetails" class="ui-widget-content" style="position:absolute; left: 10px; top: 10px; touch-action: none; width: 500px; z-index: 999; 
	...
	<div class="progress_container">
		...
		<ul style="background: #fff; border: 1px solid #c8ced3; height: 250px; overflow-y:  auto;"> 
...
var colModel = [];
GridName = "#targetGrid";
requestUrl = "${getContextPath}/search/registScheduleGroup" ; 
...
$(document).ready(function () {
	...
	colModel.push({ label: "그룹번호",  	index: "SCHEDULE_GROUP_ID", 	name: "SCHEDULE_GROUP_ID", 			width: 250, 	align: "left", hidden: true}); 
	colModel.push({ label: "스케줄명",  	index: "SCHEDULE_GROUP_NAME",	name: "SCHEDULE_GROUP_NAME",		width: "30%", 	align: "left"});
	colModel.push({ label: "스캔 대수",  	index: "SCAN_CNT", 				name: "SCAN_CNT",					width: "5%", 	align: "center"});
	colModel.push({ label: "생성자ID",  	index: "USER_NO",				name: "USER_NO",					width: 150, 	align: "center", hidden: true});
	colModel.push({ label: "스케줄작성자", 	index: "USER_NAME",				name: "USER_NAME", 					width: "10%", 	align: "center"});
	colModel.push({ label: "스캔 상태",  	index: "STATUS",				name: "STATUS", 					width: 80, 		align: "center", hidden: true });
	colModel.push({ label: "검색대상",  	index: "TYPE",					name: "TYPE", 						width: "5%", 	align: "center", formatter:createType});
	colModel.push({ label: "정책ID",  	index: "POLICY_ID",				name: "POLICY_ID", 					width: 160, 	align: "center", hidden: true});
	colModel.push({ label: "스캔 정책명",  	index: "POLICY_NAME",			name: "POLICY_NAME", 				width: "30%", 	align: "center"});
	colModel.push({ label: "데이터타입ID",	index: "DATATYPE_ID",			name: "DATATYPE_ID", 				width: 160, 	align: "center", hidden: true});
	colModel.push({ label: "데이터 타입명", index: "DATATYPE_LABEL",		name: "DATATYPE_LABEL", 			width: "40%", 	align: "center", hidden: true});
	colModel.push({ label: "생성일",  	index: "REGDATE",				name: "REGDATE",					width: "20%", 	align: "center" , type:"3_0"});
	colModel.push({ label: " ",  		index: "SCHEDULE_ID2", 			name: "SCHEDULE_ID2", 				width: "10%", 	align: "center", formatter:createView, exportcol : false, type:0, search: false, searchrules: { hidden: true }});
	colModel.push({ label: "RECON_ID",  index: "RECON_SCHEDULE_ID", 	name: "RECON_SCHEDULE_ID", 			width: "10%", 	align: "center", hidden: true});
	GridSearchTypeChk();
	searchListAppend(); 
	setSelectDate();  
	...
	$("#targetGrid").jqGrid({
	    ...
	    onCellSelect : function(rowid, icol, cellcontent, e) {
			...
			$(".statusChoiseBtn").on("click", function(e) {
				...
				if(status == 'scanning' ){
					$.ajax({
					    ...
					    success: function (resultMap) {
					    	$("#scanningDetails").html("");
					    	$.each(resultMap.resultData, function(index, item){
					    		
					    		var detailsContent = item.name + "<br>"
					    			if(item.status == "scanning"){ 
					    				detailsContent += "<label style=\"font-weight: bold; \"> " + item.status + "</label>("+ item.percentage + " 'File path ";
					    			}else{
						    			detailsContent += item.status + "(" ; 
					    			}
					    			detailsContent += item.currentlyFile + ") <br><br>"; 
					    		
					    		$("#scanningDetails").append(detailsContent);
					    	});
					...
					$.ajax({
						type: "POST",
						url: "/search/getScanPercentage",
						async : false,
						data : postData,
					    success: function (result) {
					    	var length = result.resultData
					    	$("#details_percentage").html(length);
				...
				// $("#taskWindow").css("left", (offset.left - $("#taskWindow").width() + $(this).parent().width()) + "px");
				$("#progressDetails").css("top", offset.top + $(this).height() + 93 + "px");

				var bottomLimit = $(".left_box2").offset().top + $(".left_box2").height();
				var taskBottom = Number($("#progressDetails").css("top").replace("px","")) + $("#progressDetails").height();

				if (taskBottom > bottomLimit) {  
					$("#progressDetails").css("top", Number($("#progressDetails").css("top").replace("px","")) - (taskBottom - bottomLimit) + 77 +  "px");
				} 
			...
			$.ajax({
			    ...
			    success: function (result) {
			    	var length = result.resultData; 
			    	if(length != null && length != "0.00%") $("#details_percentage").html(length);
			    },
			    error: function (request, status, error) {
			    	alert("Percentage Error");
			        console.log("ERROR : ", error);
