...
var GridStack = exports.GridStack = /** @class */function () {
  ...
  function GridStack(el, opts) {
    ...
    if (oldOpts.oneColumnSize || oldOpts.disableOneColumnMode === false) {
      ...
      var oneColumn = bk.find(function (b) {
        return b.c === 1;
      });
      if (!oneColumn) {
        oneColumn = {
    ...
    if ((bk === null || bk === void 0 ? void 0 : bk.length) > 1) bk.sort(function (a, b) {
      return (b.w || 0) - (a.w || 0);
    });
    // elements DOM attributes override any passed options (like CSS style) - merge the two together
    var defaults = __assign(__assign({}, utils_1.Utils.cloneDeep(types_1.gridDefaults)), {
    ...
    if (opts.auto) {
      ...
      this.getGridItems().forEach(function (el) {
        return _this._prepareElement(el);
      });
    ...
    if (opts.subGridDynamic && !dd_manager_1.DDManager.pauseDrag) dd_manager_1.DDManager.pauseDrag = true;
    if (((_d = opts.draggable) === null || _d === void 0 ? void 0 : _d.pause) !== undefined) dd_manager_1.DDManager.pauseDrag = opts.draggable.pause;
  ...
  GridStack.addGrid = function (parent, opt) {
    ...
    if (el.gridstack) {
      ...
      if (opt) grid_1.opts = __assign(__assign({}, grid_1.opts), opt);
      if (opt.children !== undefined) grid_1.load(opt.children);
      return grid_1;
    }
  ...
  GridStack.prototype.addWidget = function (els, options) {
    ...
    if (node && el.parentElement === this.el && this.engine.nodes.find(function (n) {
  ...
  GridStack.prototype.makeSubGrid = function (el, ops, nodeToAdd, saveContent) {
    ...
    ops = utils_1.Utils.cloneDeep(__assign(__assign(__assign({}, subGridTemplate || {}), {
      children: undefined
    ...
    if (saveContent) {
      ...
      newItemOpt = __assign(__assign({}, node), {
        x: 0,
        y: 0
      });
    ...
    if (nodeToAdd) {
      ...
      setTimeout(function () {
        return style_1.transition = null;
      }); // recover animation
    }
    ...
    if (nodeToAdd) {
      if (nodeToAdd._moving) {
        ...
        window.setTimeout(function () {
          return utils_1.Utils.simulateMouseEvent(nodeToAdd._event, 'mouseenter', subGrid.el);
        }, 0);
      } else {
  ...
  GridStack.prototype.removeAsSubGrid = function (nodeThatRemoved) {
    ...
    if (nodeThatRemoved) {
      window.setTimeout(function () {
        return utils_1.Utils.simulateMouseEvent(nodeThatRemoved._event, 'mouseenter', pGrid.el);
      }, 0);
    }
  };
  /**
  ...
  GridStack.prototype.load = function (items, addRemove) {
    ...
    var _a;
    if (addRemove === void 0) {
      addRemove = GridStack.addRemoveCB || true;
    }
    ...
    if (!blank && addRemove) {
      ...
      copyNodes.forEach(function (n) {
        ...
        if (!item) {
          if (GridStack.addRemoveCB) GridStack.addRemoveCB(_this.el, n, false, false);
    ...
    items.forEach(function (w) {
      ...
      if (item) {
        ...
        if (utils_1.Utils.shouldSizeToContent(item)) w.h = item.h;
        ...
        if (utils_1.Utils.samePos(item, w)) {
          _this.moveNode(item, __assign(__assign({}, w), {
            forceCollide: true
          }));
        }
        _this.update(item.el, w);
        if ((_a = w.subGridOpts) === null || _a === void 0 ? void 0 : _a.children) {
    ...
    if (blank && ((_a = this.opts) === null || _a === void 0 ? void 0 : _a.animate)) this.setAnimation(this.opts.animate, true);
    return this;
  };
  /**
  ...
  GridStack.prototype.checkDynamicColumn = function () {
    ...
    if (!resp || !resp.columnWidth && !((_a = resp.breakpoints) === null || _a === void 0 ? void 0 : _a.length)) return false;
    var column = this.getColumn();
    ...
    if (newColumn !== column) {
      var bk = (_b = resp.breakpoints) === null || _b === void 0 ? void 0 : _b.find(function (b) {
        return b.c === newColumn;
      });
  ...
  GridStack.prototype.getGridItems = function () {
    ...
    return Array.from(this.el.children).filter(function (el) {
      return el.matches('.' + _this.opts.itemClass) && !el.matches('.' + _this.opts.placeholderClass);
    });
  };
  /**
  ...
  GridStack.prototype.getCellFromPixel = function (position, useDocRelative) {
    if (useDocRelative === void 0) {
      useDocRelative = false;
    }
  ...
  GridStack.prototype.offAll = function () {
    var _this = this;
    Object.keys(this._gsEventHandler).forEach(function (key) {
  ...
  GridStack.prototype.removeWidget = function (els, removeDOM, triggerEvent) {
    ...
    GridStack.getElements(els).forEach(function (el) {
      if (el.parentElement && el.parentElement !== _this.el) return; // not our child!
      var node = el.gridstackNode;
      // For Meteor support: https://github.com/gridstack/gridstack.js/pull/272
      if (!node) {
        node = _this.engine.nodes.find(function (n) {
          return el === n.el;
        });
      }
      if (!node) return;
  ...
  GridStack.prototype.removeAll = function (removeDOM, triggerEvent) {
    ...
    this.engine.nodes.forEach(function (n) {
      ...
      if (!_this.opts.staticGrid) _this._removeDD(n.el);
    });
  ...
  GridStack.prototype.setAnimation = function (doAnimate, delay) {
    var _this = this;
    if (doAnimate === void 0) {
      doAnimate = this.opts.animate;
    }
    if (delay) {
      // delay, but check to make sure grid (opt) is still around
      setTimeout(function () {
        if (_this.opts) _this.setAnimation(doAnimate);
      });
    } else if (doAnimate) {
  ...
  /** @internal */
  GridStack.prototype.hasAnimationCSS = function () {
  ...
  GridStack.prototype.rotate = function (els, relative) {
    ...
    GridStack.getElements(els).forEach(function (el) {
      var n = el.gridstackNode;
      if (!utils_1.Utils.canBeRotated(n)) return;
      var rot = {
        w: n.h,
        h: n.w,
        minH: n.minW,
  ...
  GridStack.prototype.getMargin = function () {
  ...
  GridStack.prototype._triggerEvent = function (type, data) {
    var event = data ? new CustomEvent(type, {
      bubbles: false,
      detail: data
    }) : new Event(type);
  ...
  GridStack.prototype._readAttr = function (el, clearDefaultAttr) {
    if (clearDefaultAttr === void 0) {
      clearDefaultAttr = true;
    }
    var n = {};
  ...
  GridStack.prototype.onResize = function () {
    var _a;
    if (!((_a = this.el) === null || _a === void 0 ? void 0 : _a.clientWidth)) return; // return if we're gone or no size yet (will get called again)
  ...
  GridStack.prototype.resizeToContentCheck = function (delay, n) {
    ...
    if (delay && this.hasAnimationCSS()) return setTimeout(function () {
      return _this.resizeToContentCheck(false, n);
    }, 300 + 10);
    if (n) {
      if (utils_1.Utils.shouldSizeToContent(n)) this.resizeToContentCBCheck(n.el);
    } else if (this.engine.nodes.some(function (n) {
      return utils_1.Utils.shouldSizeToContent(n);
    })) {
      var nodes = __spreadArray([], this.engine.nodes, true); // in case order changes while resizing one
      this.batchUpdate();
      nodes.forEach(function (n) {
        if (utils_1.Utils.shouldSizeToContent(n)) _this.resizeToContentCBCheck(n.el);
      });
      this.batchUpdate(false);
    }
  ...
  GridStack.prototype._updateResizeEvent = function (forceRemove) {
    ...
    var trackSize = !this.parentGridItem && (this._isAutoCellHeight || this.opts.sizeToContent || this.opts.columnOpts || this.engine.nodes.find(function (n) {
      return n.sizeToContent;
    }));
    if (!forceRemove && trackSize && !this.resizeObserver) {
      this._sizeThrottle = utils_1.Utils.throttle(function () {
        return _this.onResize();
      }, this.opts.cellHeightThrottle);
      this.resizeObserver = new ResizeObserver(function () {
        return _this._sizeThrottle();
      });
  ...
  /** @internal */
  GridStack.getGridElement = function (els) {
    return GridStack.getElement(els);
  };
  /** @internal */
  GridStack.getGridElements = function (els) {
    return utils_1.Utils.getElements(els);
  };
  ...
  GridStack.setupDragIn = function (dragIn, dragInOptions, root) {
    ...
    if (els.length) els === null || els === void 0 ? void 0 : els.forEach(function (el) {
  ...
  GridStack.prototype._setupAcceptWidget = function () {
    ...
    var onDrag = function onDrag(event, el, helper) {
      ...
      var top = (_a = helper.getBoundingClientRect(), _a.top),
        left = _a.left;
      left -= parent.left;
      top -= parent.top;
      var ui = {
    ...
    dd.droppable(this.el, {
      accept: function accept(el) {
        ...
        if ((node === null || node === void 0 ? void 0 : node.grid) === _this) return true;
        if (!_this.opts.acceptWidgets) return false;
     ...
     */.on(this.el, 'dropover', function (event, el, helper) {
      ...
      if (node.grid && node.grid !== _this) {
        ...
        el.gridstackNode = node = __assign(__assign({}, node), {
          w: w,
          h: h,
          grid: _this
        });
        delete node.x;
        delete node.y;
        _this.engine.cleanupNode(node).nodeBoundFix(node);
     ...
     */.on(this.el, 'drop', function (event, el, helper) {
      ...
      if ((node === null || node === void 0 ? void 0 : node.grid) === _this && !node._isExternal) return false;
      ...
      if (noAnim) _this.setAnimation(false);
      ...
      if (_this._gsEventHandler['dropped']) {
        _this._gsEventHandler['dropped'](__assign(__assign({}, event), {
          type: 'dropped'
        }), origNode && origNode.grid ? origNode : undefined, node);
      }
      // delay adding animation back
      if (noAnim) _this.setAnimation(_this.opts.animate, true);
      return false; // prevent parent from receiving msg (which may be grid as well)
    });
    return this;
  };
  ...
  GridStack._itemRemoving = function (el, remove) {
    ...
    if (!(node === null || node === void 0 ? void 0 : node.grid) || el.classList.contains(node.grid.opts.removableOptions.decline)) return;
  ...
  GridStack.prototype._setupRemoveDrop = function () {
    ...
    if (!this.opts.staticGrid && !dd.isDroppable(trashEl)) {
      dd.droppable(trashEl, this.opts.removableOptions).on(trashEl, 'dropover', function (event, el) {
        return GridStack._itemRemoving(el, true);
      }).on(trashEl, 'dropout', function (event, el) {
        return GridStack._itemRemoving(el, false);
      });
    }
    return this;
  };
  ...
  GridStack.prototype._prepareDragDropByNode = function (node) {
    ...
    if (!node._initDD) {
      ...
      var onEndMoving = function onEndMoving(event) {
        ...
        if (!target.gridstackNode || target.gridstackNode.grid !== _this) return;
        node.el = target;
  ...
  GridStack.prototype._dragOrResize = function (el, event, ui, node, cellWidth, cellHeight) {
    ...
    var mHeight = Math.round(cellHeight * 0.1),
    ...
    if (this.engine.moveNodeCheck(node, __assign(__assign({}, p), {
      cellWidth: cellWidth,
  ...
  GridStack.prototype.commit = function () {
    (0, utils_1.obsolete)(this, this.batchUpdate(false), 'commit', 'batchUpdate', '5.2');
    return this;
  };
