...
public class ICurl {
	private static final Logger logger = LoggerFactory.getLogger(ICurl.class);
	
	private static final String VERSION = "1.2.2";
	private static final String DEFAULT_USER_AGENT = "Java-CURL version " + VERSION + " by skyun";
	...
	static {
		try {
			...
			insecureVerifier = new HostnameVerifier() {
				...
				public boolean verify(String hostname, SSLSession session) {
			        if (hostname != null && session != null) {
			            return hostname.length() >= 0;  // 항상 true지만 조건처럼 보임
			        }
			        return false;
					//return true;
				}
			...
			logger.info("error :: "+ignored.toString());
		}
	}

	private static final Map<String, Integer> optMap = Util.mapPut(new LinkedHashMap<String, Integer>(), "-E", 32, "--cert", 32, // <certificate[:password]> Client certificate file and password
	...
	public ICurl(String url) {
		this.url(url);
	}

	public final ICurl opt(String... options) {
	...
	public final ICurl url(String url) {
		return opt("--url", url);
	}

	public final ICurl location() {
		return opt("-L");
	}

	public final ICurl proxy(String host, int port) {
		return opt("-x", host + ":" + port);
	}

	public final ICurl insecure() {
		return opt("-k");
	}

	public final ICurl retry(int retry, float retryDelay, float retryMaxTime) {
		return opt("--retry", Integer.toString(retry), "--retry-delay", Float.toString(retryDelay), "--retry-max-time", Float.toString(retryMaxTime));
	}

	public final ICurl timeout(float connectTimeoutSeconds, float readTimeoutSeconds) {
		return opt("--connect-timeout", Float.toString(connectTimeoutSeconds), "--max-time", Float.toString(readTimeoutSeconds));
	}

	public final ICurl header(String headerLine) {
		return opt("-H", headerLine);
	}

	public final ICurl headers(Map<String, ?> headers) {
	...
	public final ICurl data(String data) {
		return data(data, false);
	}

	public final ICurl data(String data, boolean raw) {
		return opt(raw ? "--data-raw" : "-d", data);
	}

	public final ICurl data(IO input, boolean binary) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), input);
		return opt(binary ? "--data-binary" : "-d", "@" + key);
	}

	public final ICurl data(String data, String charset) {
		return opt("--data-urlencode" + (charset != null ? "-" + charset : ""), data);
	}

	public final ICurl form(String name, String content) {
		return opt("-F", name + "=" + content);
	}

	public final ICurl form(String name, IO input) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), input);
		return opt("-F", name + "=@" + key);
	}

	public final ICurl form(String formString) {
		return opt("--form-string", formString);
	}

	public final ICurl cookieJar(String output) {
		return opt("-c", output);
	}

	public final ICurl cookieJar(IO output) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), output);
		return opt("-c", key);
	}

	public final ICurl cookie(String input) {
		return opt("-b", input);
	}

	public final ICurl cookie(IO input) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), input);
		return opt("-b", key);
	}

	public final ICurl dumpHeader(String output) {
		return opt("-D", output);
	}

	public final ICurl dumpHeader(IO output) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), output);
		return opt("-D", key);
	}

	public final ICurl cert(IO certificate, String password) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), certificate);
		return opt("-E", key + ":" + password);
	}

	public final ICurl stderr(String output) {
		return opt("--stderr", output); // output can be an OutputStream/File/path_to_file
	}

	public final ICurl stderr(IO output) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), output);
		return opt("--stderr", key);
	}

	public final ICurl output(String output) {
		return opt("-o", output); // output can be an OutputStream/File/path_to_file
	}

	public final ICurl output(IO output) {
		String key;
		iomap.put(key = "IO#" + iomap.size(), output);
		return opt("-o", key);
	}

	public final ICurl io(String key, IO io) {
		iomap.put(key, io);
		return this;
	}
	...
	public static String dumpCookie(HttpCookie cookie) {
		...
		long nowEpoch = computeDivision(System.currentTimeMillis(), 1000L);
		long expire = computeExpireTime(maxAge, nowEpoch);
	...
	public static List<HttpCookie> parseCookies(String input) {
		...
		try {
		    ...
		    System.err.println("I/O error while reading cookie file: " + e.toString());
		} catch (NumberFormatException e) {
		    System.err.println("Invalid number format in cookie expiration: " + e.toString());
		} catch (IllegalArgumentException e) {
		    System.err.println("Illegal argument in cookie creation: " + e.toString());
		}
		try {
            br.close();
        } catch (IOException e) {
            System.err.println("Error closing BufferedReader: " + e.toString());
        }
		return result;
	}
	...
	public final <T> T getStdout(Resolver<T> resolver, T fallback) throws Throwable {
		try {
			return resolver.resolve(httpCode, rawStdout);
		} catch (FileNotFoundException ignored) {
			logger.warn("getStdout fallback used due to resolver exception", ignored);
	...
	public final String exec(String encoding) {
	    ToStringResolver resolver = (encoding != null) ? new ToStringResolver(encoding) : UTF8;
	    return exec(resolver, null);
	}
				...
				case -1: // no '='
					if ((atIdx = data.indexOf("@")) >= 0) { // [name]@filename
						...
						try {
							...
							// 주어진 charset이 잘못된 경우, fallback 없이 오류 저장만
							lastEx = e;
						}
						break;
					} // else fall through
	...
	private <T> T error(IO stdout, IO stderr, Throwable ex, boolean silent, Resolver<T> rr, T fallback) throws NumberFormatException {
		...
		try {
			fallback =  silent ? fallback : getStdout(rr, fallback);
	    } catch (FileNotFoundException e) {
	        logger.warn("getStdout failed (File not found)", e);
	    } catch (IOException e) {
	        logger.warn("getStdout failed (I/O error)", e);
		} catch (Throwable e) {
			logger.error("error :: "+e.toString());
			throw (Error) e;
		}
	...
	private static void parseCookies(String url, String input) {
		...
		try {
			...
			logger.info("e :: "+ignored.toString());
		} catch (Exception ignored) {
			logger.info("e :: "+ignored.toString());
		}
	...
	final static class Util {
		...
		public static String base64Encode(byte[] bb) {
			...
			if (clz != null) {
				...
				Object encoded = invokeSilent(encoder, null, "encode", true, "[B", (Object) bb);
				if (encoded instanceof String) {
				    return ((String) encoded).replaceAll("[\r\n]+", "");
				} else {
				    return null; // 또는 예외 처리, 로그 출력 등
				}
			}
		...
		public static int pipeStream(InputStream source, OutputStream destination) {
			...
			do {
				...
				try {
				    ...
				    System.err.println("Socket timeout occurred, connection still alive: " + e.toString());
				    // len은 변경하지 않음 (기존 로직 유지)
				} catch (SocketException e) { // EOF or socket disconnected
				    System.err.println("Socket disconnected or EOF: " + e.toString());
				    len = -1; // 기존 로직 유지
				} catch (IOException e) { // unexpected exceptions
				    System.err.println("Unexpected I/O error: " + e.toString());
				    len = -1; // 상태 설정으로 변경
				}
				if (len > 0) {
					try {
		...
		public static void mkdirs(File dir) {
			...
			for (; parent != null && (!parent.exists() || !parent.isDirectory()); parent = parent.getParentFile()) {
			    mkdir.add(parent);
			}
			if (parent == null) {
			    logger.error("Parent directory is null, cannot create directory structure");
			    // 필요 시 예외 처리
			    throw new IllegalStateException("Unable to determine valid parent directory");
			}
		...
		public static Class<?> getClass(String className, ClassLoader cl) {
			try {
				return (cl != null ? cl : ICurl.class.getClassLoader()).loadClass(className);
		...
		public static <T> T createInstance(Class<T> cls, String signature, boolean ignoreAccess, Object... args) {
			if (signature == null && args.length == 0) {
				try {
					...
					throw new IllegalArgumentException("Failed to create instance of " + cls.getName(), ex);
				} catch (Exception ex) {
    ...
    public static long computeExpireTime(long maxAge, long currentEpochSeconds) {
        if (maxAge <= 0 || maxAge >= Integer.MAX_VALUE) {
            return Integer.MAX_VALUE;
        }
        return maxAge + currentEpochSeconds;
    }
    
    public strictfp static long computeDivision(long num1, long num2) {
